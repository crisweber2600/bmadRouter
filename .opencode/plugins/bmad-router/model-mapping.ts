// Model mapping from NotDiamond model names to OpenCode-compatible model names
...
const GITHUB_COPILOT_MODEL_MAPPING: Record<string, string> = {
 ...
 */
export function mapToOpenCodeModel(provider: string, model: string): string {
  return model;
}
 ...
 */
export function mapProviderToOpenCode(provider: LLMProvider): LLMProvider {
  return {
