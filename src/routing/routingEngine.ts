import { LLMRequest, ModelTier, RoutingDecision, RoutingSignals, DecisionPath, ProviderName } from '../types';
...
export class RoutingEngine {
  ...
  private readonly CASCADE_THRESHOLD = 0.7; // Below this, cascade to higher tier
  private readonly CONSENSUS_THRESHOLD = 0.85; // Above this, high confidence decision
  private readonly MIN_CONFIDENCE = 0.5; // Minimum acceptable confidence

  async makeDecision(request: LLMRequest, signals: RoutingSignals): Promise<RoutingDecision> {
    ...
    try {
      ...
      if (request.bmadRouter?.tier) {
        ...
        logger.info('Override applied', { tier: overrideDecision.selectedTier });
        return overrideDecision;
      }

      // Check for bypass flag - go straight to PREMIUM
      if (request.bmadRouter?.bypass) {
        logger.info('Bypass mode - routing to PREMIUM');
      ...
      logger.error('Routing decision failed, using fallback', { error });
   ...
   */
  private applySemanticCascading(signals: RoutingSignals): CascadeResult {
    let attempts = 1;
    ...
    logger.debug('Cascade Phase 1 - Initial assessment', {
      tier: initialTier,
    ...
    if (currentConfidence < this.CASCADE_THRESHOLD) {
      ...
      logger.debug('Cascade Phase 3 - Cascading up', {
    ...
    if (currentConfidence < this.CASCADE_THRESHOLD) {
      ...
      logger.debug('Cascade Phase 4 - Consensus', {
   ...
   */
  private assessTierFromSignals(signals: RoutingSignals): ModelTier {
   ...
   */
  private calculateTierConfidence(tier: ModelTier, signals: RoutingSignals): number {
   ...
   */
  private cascadeToHigherTier(currentTier: ModelTier, signals: RoutingSignals): { tier: ModelTier; confidence: number } {
    const tierOrder: ModelTier[] = ['FREE', 'CHEAP', 'BALANCED', 'PREMIUM'];
   ...
   */
  private applyMultiSignalConsensus(signals: RoutingSignals): { tier: ModelTier; confidence: number } {
    ...
    for (const [tier, score] of Object.entries(tierScores)) {
      if (tier !== 'FALLBACK' && tier !== 'FREE' && score > maxScore) {
        maxScore = score;
        selectedTier = tier as ModelTier;
      }
    }
  ...
  private selectModelFromTier(tier: ModelTier): string {
  ...
  private createDecision(tier: ModelTier, confidence: number, path: DecisionPath): RoutingDecision {
  ...
  private async handleOverride(request: LLMRequest, signals: RoutingSignals): Promise<RoutingDecision> {
    const requestedTier = request.bmadRouter!.tier as ModelTier;
    const reason = request.bmadRouter!.reason || 'User override';

    logger.info('Processing override request', { requestedTier, reason });

    // Validate requested tier
    if (!this.tierMappings[requestedTier]) {
      logger.warn('Invalid tier requested, using heuristic cascade', { requestedTier });
      const cascadeResult = this.applySemanticCascading(signals);
  ...
  async makeFallbackDecision(): Promise<RoutingDecision> {
    logger.warn('Using fallback routing decision');
  ...
  updateTierMapping(tier: ModelTier, models: string[]): void {
    ...
    logger.info('Tier mapping updated', { tier, models });
  }
