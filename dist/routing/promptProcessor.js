...
class PromptProcessor {
    async processPrompt(request) {
        ...
        try {
            ...
            logger_1.logger.debug('Prompt processed', { processingTime, signals });
            return signals;
        }
        catch (error) {
            logger_1.logger.error('Prompt processing failed', { error });
    ...
    calculateComplexity(request) {
        ...
        for (const message of request.messages) {
            ...
            const codeCount = codeMarkers.reduce((count, marker) => count + (content.includes(marker) ? 1 : 0), 0);
            ...
            const reasoningCount = reasoningWords.reduce((count, word) => count + (content.toLowerCase().includes(word) ? 1 : 0), 0);
    ...
    detectCode(request) {
        ...
        return request.messages.some(message => codePatterns.some(pattern => pattern.test(message.content)));
    }
    ...
    calculateConfidence(complexity, hasCode, intentCategory) {
        // Base confidence from complexity analysis
        let confidence = 0.7; // Base confidence
        // Adjust for clear signals
        if (hasCode)
            confidence += 0.2; // Code detection is very reliable
        if (intentCategory !== 'mixed')
            confidence += 0.1; // Clear intent classification
